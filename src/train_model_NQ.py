...
def train_model(
    ...
    """
    Progressive fine-tuning of MobileNetV2 in 3 stages (float model, no binarization):

    1) Train classifier only
    2) Unfreeze deeper blocks (features.15–18 + classifier)
    3) Unfreeze final blocks (features.8–18 + classifier) for polishing

    Includes per-epoch validation, optional ReduceLROnPlateau, and per-stage early stopping.

    Args:
        data_is_quantized (bool): Used only to choose checkpoint filename (bookkeeping).
        stage_epochs (tuple[int,int,int]): Epochs per stage.
        early_stop_patience (int): Stop stage when val loss does not improve for N epochs.
        use_lr_on_plateau (bool): Enable ReduceLROnPlateau on validation loss.
        train_loader (DataLoader): Yields (images, labels). Preprocessing done upstream.
        val_loader (DataLoader): Yields (images, labels). Preprocessing done upstream.
    """
    ...
    # Each stage specifies which layers to unfreeze, epochs, and learning rate
    # Note: matching is substring-based (e.g., "features" matches all feature blocks).
    stages = [
    ...
    # Internal helper: run training for one stage
    def run_training(model, stage, stage_idx):
        ...
        # Freeze all parameters, selectively unfreeze per stage
        for p in model.parameters():
            p.requires_grad = False

        # Unfreeze only the layers required for this stage (substring matching)
        for name, p in model.named_parameters():
            if any(layer in name for layer in stage["layers"]):
                p.requires_grad = True

        # Define loss, optimizer, and optional LR scheduler
        criterion = nn.CrossEntropyLoss()
        optimizer = optim.Adam((p for p in model.parameters() if p.requires_grad), lr=stage["lr"])
        scheduler = None
        if use_lr_on_plateau:
            # Reduce LR if validation loss plateaus; cooldown lets it stabilize after a cut
        ...
        for epoch in range(stage["epochs"]):
            ...
            for xb, yb in train_loader:
                ...
                loss.backward()
                optimizer.step()
                # Accumulate stats scaled by batch size for correct epoch averages
    ...
    print("="*50)
    evaluate(model, val_loader, device, name="Final Validation")
