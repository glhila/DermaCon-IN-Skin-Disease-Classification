...
from train_model_Q import convert_mobilenetv2_to_bnn
...
def load_model_for_eval(model_path: str, mode: str, device: torch.device, num_classes: int = 2) -> nn.Module:
    """Load a model from a saved artifact given the mode.
...
def evaluate_saved_model(model_path: str, mode: str = "fp32", name: str = "Test", test_loader :DataLoader = None) -> None:
    """High-level helper: load model from path and evaluate.

    Args:
        model_path (str): Path to weights/module to load.
        mode (str): One of {"fp32", "bnn_state", "qat_state", "int8_module", "int8_state", "dynamic_linear"}.
        name (str): Label for printed metrics.
        test_loader (DataLoader | None): Dataloader yielding (images, labels). If None, this function will raise.
    """
